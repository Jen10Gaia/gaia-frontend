...
const JobCandidates = ({ candidatesApplied, access_token }) => {
  const { clearErrors, error, loading, approveApplication, setApproved, approved,  positionsLeft,  } = useContext(JobContext);
  ...
  candidatesApplied &&
    candidatesApplied.forEach((item) => {
      data.push({
        ...
        lastDate: item.job.lastDate.substring(0, 10),
        status: item.status,
        resume: (
          <>
            <Link
        ...
        action: (
          <>
            {positionsLeft < item.job.positions & !item.approved & !item.rejected ? (
              <button
