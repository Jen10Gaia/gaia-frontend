...
export const AuthProvider = ({ children }) => {
  ...
  const [bankStatementsUploaded, setBankStatementsUploaded] = useState(null);
  const [academicPapersUploaded, setAcademicPapersUploaded] = useState(null);
  ...
  const uploadBankStatements = async (formData, access_token) => {
    try {
      setLoading(true);

      const res = await axios.put(
        `${process.env.API_URL}/api/upload/bank-statements/`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${access_token}`,
      ...
      if (res.data) {
        setLoading(false);
        setBankStatementsUploaded(true);
      }
    } catch (error) {
      console.log(error.response);
      setLoading(false);
  ...
  const uploadAcademicPapers = async (formData, access_token) => {
    try {
      setLoading(true);

      const res = await axios.put(
        `${process.env.API_URL}/api/upload/academic-documents/`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${access_token}`,
      ...
      if (res.data) {
        setLoading(false);
        setAcademicPapersUploaded(true);
      }
    } catch (error) {
      console.log(error.response);
      setLoading(false);
